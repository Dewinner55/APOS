...
const EditCategoryInfo = ({ category, languages, open, setOpen, showSuccessMessage }: Props) => {
  ...
  const handleEditSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!currentLangData.content.title && !currentLangData.content.description) {
      console.log('Все поля пустые. Невозможно изменить категорию.');
    ...
    try {
      ...
      if (editCategoryImage && (!fileExtension || !['jpg', 'jpeg', 'png', 'gif'].includes(fileExtension))) {
        throw new Error('Неверный формат файла. Разрешены только файлы JPG, JPEG, PNG или GIF.');
      }
      ...
      if (fileUploadSuccess || createdSuccess) {
        showSuccessMessage('Категория успешно изменена.');
      ...
      >
        ...
        <Box sx={{ mt: 2 }}>
          <Typography component='span' className='flex flex-col text-center'>
            Обновление сведений о категории будет подвергнуто аудиту конфиденциальности.
          </Typography>
        </Box>
        <IconButton
          onClick={handleReset}
          className='absolute block-start-4 inline-end-4'
          style={{ position: 'absolute', top: 10, right: 10 }} // добавляем стили для позиционирования иконки
        >
      ...
      <form onSubmit={handleEditSubmit}>
        ...
        <DialogActions className='gap-2 justify-center pbs-0 pbe-10 pli-10 sm:pbe-16 sm:pli-16'>
          ...
          <StyledSnackbar
            ...
            message='All fields are empty. Cannot create category.'
          />
        </DialogActions>
      </form>
