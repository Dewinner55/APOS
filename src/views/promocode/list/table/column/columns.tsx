...
type CategoryTypeWithAction = Category & {
  action?: string
}
...
export const createColumns = (
  ...
  [
    {
      id: 'select',
      header: ({table}: { table: Table<CategoryTypeWithAction> }) => (
        <Checkbox
          {...{
      ...
      cell: ({row}: { row: Row<CategoryTypeWithAction> }) => (
        <Checkbox
          {...{
    ...
    columnHelper.accessor('action', {
      ...
      cell: ({row}) => (
        <div className='flex items-center'>
          <Link href={`http://localhost:3009/category/view/${row.original.id}`}>
            <IconButton sx={eyeIconStyle}>
              <i className='ri-eye-line text-[22px] text-textSecondary'/>
            </IconButton>
          </Link>
          <IconButton onClick={() => handleEditCategory(row.original)} sx={editIconStyle}>
            <i className='ri-edit-box-line text-[22px] text-textSecondary'/>
          </IconButton>
          <IconButton onClick={() => handleDeleteCategory(row.original)} sx={deleteIconStyle}>
